...
| OpenSTAT | `uv run python main.py openstat` | `OPENSTAT=true`, `FLARESOLVERR_URL`; optional `OPENSTAT_URLS` override | `data/openstat_processed/openstat_table.csv`, `data/openstat_processed/openstat_corpus.jsonl`, checkpoint | Scrape/download completes; table + CPT JSONL generated; no fatal parse errors |
