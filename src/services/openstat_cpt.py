"""OpenSTAT tabular data → optional CPT sentence records.

Primary output is ``openstat_table.csv`` (structured price data).
Line-level JSONL is optional CPT/training output only — not indexed into RAG.
"""
...
WRITE_LINE_CORPUS = os.getenv("OPENSTAT_WRITE_LINE_CORPUS", "false").strip().lower() in (
    "true",
    "1",
    "yes",
)
...
def run(df: pd.DataFrame | None = None, *, csv_path: Path | None = None) -> int:
    """Persist OpenSTAT table CSV and optionally write line-level CPT JSONL."""
    console.rule("[bold cyan]OpenSTAT – Tabular export[/bold cyan]")

    if df is None:
        source_csv = csv_path or OPENSTAT_TABLE_CSV
        console.print(f"[dim]Input CSV: {source_csv}[/dim]")
        if not source_csv.is_file():
            console.print("[yellow]No OpenSTAT table CSV found; skipping export step.[/yellow]")
            return 0
    ...
    if not WRITE_LINE_CORPUS:
        console.print(
            "[dim]OPENSTAT_WRITE_LINE_CORPUS=false — table CSV only (not RAG-indexed).[/dim]"
        )
        return len(df)
    ...
    console.print(f"[dim]Line-level CPT (optional): {out}[/dim]")
    console.print(f"[green]CPT sentences (1 row = 1 line):[/green] {len(line_records)}")
